...
class Opec(Provider):
    ...
    def _fetch_payments(self) -> list[Payment]:
        ...
        self._browser.find_element(By.XPATH, PAYMENTS_TAB).click()
        self._weblogger.trace("pre-documents-click")
        self._browser.find_element(By.XPATH, DOCUMENTS_TAB).click()
        self._browser.wait_for_network_inactive()
        sleep(1)

        invoices = self._browser.find_element(By.TAG_NAME, TABLE_BODY).find_elements(By.TAG_NAME, TABLE_ROW)
        amount = self._browser.find_element(By.NAME, AMOUNT_FIELD_NAME).text
        due_date = ''
