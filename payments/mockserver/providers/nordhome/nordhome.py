"""Nordhome blueprint for the local mock portal."""
...
bp = Blueprint("nordhome", __name__)
...
@bp.get("/")
def root() -> Response:
    """Redirect the blueprint root to the Nordhome login page."""
    return redirect(url_for("nordhome.nordhome_login"))


@bp.get(f"{BASE_PATH}/")
def nordhome_index() -> Response:
    """Mirror the portal base path and forward it to the login page."""
    return redirect(url_for("nordhome.nordhome_login"))


@bp.get(f"{BASE_PATH}/login")
def nordhome_login() -> str:
...
def nordhome_mock_login() -> Response:
    ...
    if scenario == "error":
        return redirect(url_for("nordhome.nordhome_login", scenario="error", _anchor="mainbox"))

    if not username or not password:
        return redirect(url_for("nordhome.nordhome_login", scenario="error", _anchor="mainbox"))

    return redirect(url_for("nordhome.nordhome_home", scenario=scenario))


@bp.get(f"{BASE_PATH}/home")
def nordhome_home() -> str:
...
def _render_login_page(*, scenario: str) -> str:
    ...
    _append_helper_script(
        soup,
        """
        document.title = 'Nordhome mock - login';
